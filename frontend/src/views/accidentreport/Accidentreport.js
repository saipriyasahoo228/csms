...
const headCells = [
  { id: 'name', numeric: false, disablePadding: true, label: 'Accident Type' },
...
function EnhancedTableToolbar(props) {
  const { numSelected, onPdfDownload, onSearchChange, searchTerm } = props;
...
export default function Accidentreport() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get('/reports/organization/accidents/');
  ...
  const visibleRows = React.useMemo(() => {
    const filteredRows = rows.filter(row =>
      ...
      row.number.toString().includes(searchTerm.toLowerCase())
    );
  ...
  const handlePdfDownload = () => {
    const doc = new jsPDF();
    doc.autoTable({
      head: [['Accident Type', 'Number']],
      body: visibleRows.map((row) => [row.name, row.number]),
    });
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="right" padding="5px">
                      {row.number}
                    </TableCell>
                  </TableRow>
                );
