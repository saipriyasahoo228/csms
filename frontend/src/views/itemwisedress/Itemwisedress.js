...
export default function Itemwisedress() {
  ...
  useEffect(() => {
          const fetchItem = async () => {
            try {
              ...
              const filteredData = data.filter(item => item.item_type.item_type_id === 3);
  ...
  useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await api.get('/item/issued/?filter=Dress');
  ...
  const handleChangeRowsPerPage = (event) => {
   ...
   const handlePdfDownload = () => {
    ...
    const handleExcelDownload = async () => {
      ...
      const header = ['Employee Code', 'Employee Name', ...headCells1.map(cell => cell.label)];
      ...
      worksheet.columns.forEach(col => {
        const maxLength = Math.max(...col.values.map(value => (value ? value.toString().length : 0)));
  ...
  const filteredRows = rows.filter((row) =>
    ...
    row.dress.toLowerCase().includes(searchQuery.toLowerCase())||
    row.jacket.toLowerCase().includes(searchQuery.toLowerCase())||
    row.reflectingjacket.toLowerCase().includes(searchQuery.toLowerCase())||
    row.raincoat.toLowerCase().includes(searchQuery.toLowerCase())
  );
