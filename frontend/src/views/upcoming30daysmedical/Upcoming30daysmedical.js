...
export default function Upcoming30daysmedical() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get('/medical/upcoming/30/');
  ...
  const handlePdfDownload = () => {
    ...
    doc.save('Upcoming 30 days medical-checkup-reports.pdf');
  };
            ...
            <TableBody>
              {visibleRows.map((row) => (
                ...
                >
                  ...
                  {/* <TableCell align="right">{row.checkup_image}</TableCell> */}
                </TableRow>
              ))}
              {emptyRows > 0 && (
